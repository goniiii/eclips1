...
//		myHome.addElcApp(air);		
		...
		myHome.addElcApp(new Aircon("거실가전", "스마트에어컨", 220));
		myHome.addElcApp(new BluetoothSpeaker("소형가전", "스마트브리츠", 220));
		myHome.addElcApp(new RobotVacuum("스마트가전", "둥글이", 220));

//		myHome.showElcAppList(); // 이 집에 있는 가전 리스트를 보고싶다. myHome 안에 구현해보기
		

		SmartAircon smAir = new SmartAircon("거실가전", "스마트 에어컨", 220);
		SmartBluetoothSpeaker smBs = new SmartBluetoothSpeaker("소형가전", "스마트 브리츠", 220);
		SmartRobotVacuum smRv = new SmartRobotVacuum("스마트가전", "스마트 둥글이", 220);
		...
		myHome.runSmartMachine(); // 모든 기기 작동 명령
